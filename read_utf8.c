...
int *utf8ToUnicode(int *hbp, int *hbp_end)
{
    ...
    while (hbp < hbp_end)
    {
        ...
        for (int i = 1; i < BYTE_COUNTER; i++)
        {
            ...
            free(strbinCur); // sometimes crashs the program. unkown reason
        }
        ...
        *(struni + len_struni) = '\0';
        ...
        free(strbinStart); // sometimes crashs the program. unkown reason
        free(struni); // sometimes crashs the program. unkown reason
    }
...
void read3()
{
    ...
    free(bytech); // sometimes crashs the program. unkown reason
    free(byte_arr); // sometimes crashs the program. unkown reason
